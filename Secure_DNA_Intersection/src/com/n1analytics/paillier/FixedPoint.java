...
public class FixedPoint {
  ...
  public static FixedPoint encode(double value, int exponent) {
    if (exponent < FixedPoint.DOUBLE_MIN_VALUE_EXPONENT) // Should this be an error?
    {
      throw new IllegalArgumentException(
              "exponent must be greater than or equal to DOUBLE_MIN_EXPONENT");
    }
    if (exponent > FixedPoint.DOUBLE_MAX_VALUE_EXPONENT) {
      throw new IllegalArgumentException(
              "exponent must be less than or equal to DOUBLE_MAX_EXPONENT");
    }
    ...
    if (valueExponent > 0) {
      // Normalised number
      encoding = BigInteger.valueOf(
              valueSign * (valueSignificand | 0x0010000000000000L)).shiftLeft(
              DOUBLE_MIN_VALUE_EXPONENT + valueExponent - exponent - 1);
    } else if (valueSignificand > 0) {
      // Subnormal number
      encoding = BigInteger.valueOf(valueSign * valueSignificand).shiftLeft(
              DOUBLE_MIN_VALUE_EXPONENT - exponent);
    } else {
      // Zero
  ...
  public FixedPoint changeExponent(int newExponent) {
    return new FixedPoint(significand.shiftRight(newExponent - exponent), newExponent);
  }
}
